...
AddrMut Cstl_Slice_get_unchecked(
    Cstl_Slice const self, usize const index
) {
    return (u8 mut*) self.ptr + elem_size_of(self.meta) * index;
}

AddrMut Cstl_Slice_get(Cstl_Slice const self, usize const index) {
...
Cstl_Slice Cstl_Slice_from_raw(
    AddrMut const ptr, usize const len, usize const elem_size
) {
    return (Cstl_Slice) {
        .ptr = ptr,
        .len = len,
...
void Cstl_Slice_sort_unstable(
    Cstl_Slice const self, Cstl_Comparator const cmp
) {
    if (self.len <= 1) {
        return;
    } else if (2 == self.len) {
    ...
    Cstl_Slice_sort_unstable(left, cmp);
    Cstl_Slice_sort_unstable(right, cmp);
}
...
void Cstl_Slice_binary_insert_sort(
    ...
    while (sorted.len < self.len) {
        usize mut insert_idx = Cstl_Slice_binary_search(
            sorted, Cstl_Slice_get_unchecked(self, sorted.len), cmp
        );
...
usize Cstl_Slice_binary_search(
    Cstl_Slice const self, Addr const value_ptr, Cstl_Comparator const cmp
) {
...
usize Cstl_Slice_partition_at(
    Cstl_Slice const self, usize mut pivot_idx, Cstl_Comparator const cmp
) {
...
void Cstl_Slice_swap(Cstl_Slice const self, const usize lhs, const usize rhs) {
...
void Cstl_Slice_swap_unchecked(
    Cstl_Slice const self, usize const lhs, usize const rhs
) {
    Cstl_swap(
        (u8 mut*) self.ptr + elem_size_of(self.meta) * lhs,
        (u8 mut*) self.ptr + elem_size_of(self.meta) * rhs,
        elem_size_of(self.meta)
    );
}

Cstl_Slice Cstl_Slice_slice_unchecked(
    Cstl_Slice const self, usize const start, usize mut end
) {
    if (end < start) {
        end = start;
    }
...
Cstl_Slice Cstl_Slice_slice(
    Cstl_Slice const self, usize const start, usize const end
) {
...
Cstl_Ordering Cstl_Slice_cmp(
    Cstl_Slice const lhs, Cstl_Slice const rhs, Cstl_Comparator const cmp
) {
    ...
    while (Cstl_Ordering_Equal == cmp(
        (u8 mut*) lhs.ptr + elem_size * ne_idx,
        (u8 mut*) rhs.ptr + elem_size * ne_idx
    )) {
        ne_idx += 1;
    }
...
Cstl_Ordering Cstl_Slice_cmp_elems_unchecked(
    Cstl_Slice const self, usize const lhs,
    usize const rhs, Cstl_Comparator const cmp
) {
    return cmp(
...
void Cstl_Slice_heapify(
    Cstl_Slice const self, usize const index,
    Cstl_Ordering const target_order, Cstl_Comparator const cmp
) {
...
void Cstl_Slice_build_heap(
    Cstl_Slice const self, Cstl_Ordering const target_order,
...
void Cstl_Slice_max_heapify(
    Cstl_Slice const self, usize const index, Cstl_Comparator const cmp
) {
...
void Cstl_Slice_reverse(Cstl_Slice const self) {
    for (usize mut i = 0; i < self.len / 2; ++i) {
        Cstl_Slice_swap_unchecked(self, i, self.len - i - 1);
    }
}
